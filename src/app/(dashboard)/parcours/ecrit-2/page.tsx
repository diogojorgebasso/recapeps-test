...
export default function Ecrit2Page() {
    const [subjects, setSubjects] = useState<SubjectNote[]>([]);
    const { isSubscribed } = useSubscription();

    useEffect(() => {
        const loadSubjects = async () => {
            const allSubjects = await getNotes('ecrit-2'); // Pass collection name 'ecrit-2'
            setSubjects(allSubjects.filter((subject) => subject.evaluation === 2));
